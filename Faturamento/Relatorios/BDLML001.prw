...
dbSelectArea("NNS")
dbSetOrder(1)
dbSeek(xFilial("NNS") + _cDocTrans)
if NNS->(fieldPos("NNS_XEVNTF")) > 0 .and. NNS->NNS_XEVNTF != "S"
	...
	//Flag para indicar que foi enviado o workflow
	Reclock("NNS", .F.)
	NNS->NNS_XEVNTF := "S"
	MsUnlock()	
endif
...
if PswSeek(_cUserId, .T. )  
   cEmail := PswRet()[1][14] + SuperGetMv("MV_XENVMAI", .F., ";juarez.henrique@bdil.com.br;vagner.venceslau@bdil.com.br;expedicao@bdil.com.br")  // Retorna vetor com informa��es do usu�rio
endif
...
_cHtm += '<html>'
_cHtm += '<body>'
_cHtm += '	<h1><u>Resumo de Atendimento - Solicita&ccedil;&atilde;o De Transfer&ecirc;ncia</u></h1>'
_cHtm += '	<h2>Informamos que a solicita&ccedil;&atilde;o de transfer&ecirc;ncia n&deg; ' + _cDocTrans + ' realizada em ' + DtoC(NNS->NNS_DATA) + ' para o armaz&eacute;m ' + (_cAlias)->NNT_LOCLD + ' foi atendida.</h2>'
_cHtm += '  <h2>Abaixo esta o comparativo entre a quantidade solicitada e a quantidade enviada.*</h2>'
...
_cHtm += '	</table>'
_cHtm += '	<p><i>* Informamos que a quantidade enviada pode ser diferente da solicitada devido a erro na solicita&ccedil;&atilde;o, disponibilidade do item em estoque ou descontinuidade do seu uso.</i></p>'
_cHtm += '</body>'
_cHtm += '</html>'
