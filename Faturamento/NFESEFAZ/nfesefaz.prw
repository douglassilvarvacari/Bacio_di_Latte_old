...
Local cMVCFOPREM	:= AllTrim(GetNewPar("MV_CFOPREM",""))     // Parï¿½metro que informa as CFOPs de Remessa para entrega Futura que terï¿½o tratamento para que o valor de IPI seja considerado como Outras Despesas Acessï¿½rias (tag vOutros).
...
local cMVREFNFE		:= SuperGetMV("MV_REFNFE", ," ") 			// Parametro para informe quais CFOPs sï¿½o de simples Remessa para levar informaï¿½ï¿½o 
Local cMVCfopTran	:= SuperGetMV("MV_CFOPTRA", ," ")   		// Parametro que define as CFOPï¿½s pra transferï¿½ncia de Crï¿½dito/Dï¿½bito
Local cCliLoja		:= "" 
Local cCliNota		:= ""
...
Local cChaveD2		:= ""
Local cChaveF2		:= ""
Local cTpOper		:= "" //Tipo de operaï¿½ï¿½o complemento
Local cChaveD1		:= "" 
Local cMVAEHC 		:= AllTrim(GetNewPar("MV_AEHC",""))     // Informar o cï¿½digo de classificaï¿½ï¿½o AEHC
Local cHoraNota		:= ""
Local cIndPres		:= ""
Local cIndIss		:= ""
Local cFilDev		:= ""		//Guarda filial de devoluï¿½ï¿½o
Local cMsgDI		:= ""
Local cTpGar		:= SuperGetMV("MV_LJTPGAR",,"GE")
Local cFieldMsg	:= ""
Local cSpecie		:= ""
Local cChCupom	:= ""
Local cDevMerc	:= "" //Identifica devoluï¿½ï¿½o de mercadoria que nï¿½o foi entregue ao destinatï¿½rio em atendimento ao Artigo 453, I, do RICMS/2000 SP)
Local cEndEmit	:= ""
Local cFoneEmit	:= ""
Local cCodlan		:= ""
Local cMunTransp	:= ""
...
Local nDescRed  	:= 0   			// Valores dos descontos dos itens referente ao Decreto nï¿½ 43.080/2002 RICMS-MG (SFT)
Local nDesTotal  	:= 0   			// Valor total dos descontos referente ao Decreto nï¿½ 43.080/2002 RICMS-MG
...
Local nDescNfDup	:= 0   			// Valores dos descontos para deduzir os valores de ICMS diferido na NF e da Duplicata (opï¿½ï¿½o: 6 ï¿½ Diferido(Deduz NF e Duplicata)	
Local nPercLeite	:= 0	  			//Percentual da reduï¿½ï¿½o do Leite	
Local nValLeite		:= 0   			//Valor da reduï¿½ao do Leite
Local nPrTotal		:= 0   
Local nCont	 		:= 0
Local nValBse		:= 0
Local nValIss		:= 0
...
//Declaraï¿½ï¿½o de Lï¿½gicos
Local lQuery    	:= .F.
Local lCalSol		:= .F.
Local lOk			:= .T.
Local lBrinde		:= .F.							// Flag que define se ï¿½ uma operaï¿½ï¿½o de Brinde
Local lContinua		:= .T.
Local lCabAnf		:= .T.
Local lConsig   	:= .F.								// Flag que diz se a operaï¿½ï¿½o ï¿½ de consignaï¿½ï¿½o mercantil
Local lNfCup		:= .F.								// Define se eh Nf sobre cupom
Local lNFPTER		:= GetNewPar("MV_NFPTER",.T.)					
Local lComplDev		:= .F.		   	  					//Utilizado para identificar quando for uma nota de complemento de IPI de uma devuluï¿½ï¿½o.
Local lIpiDev   	:= GetNewPar("MV_IPIDEV",.F.)   //Apenas para devoluï¿½ï¿½o de compra de IPI (nota de saï¿½da). T-Sï¿½ra gerado na tag vIPI e destacado no campo
														//VALOR IPI do cabeï¿½alho do danfe. F-Serï¿½ gerado na tag vOutro e destacado nas informaï¿½ï¿½es complementares do danfe
...
local lIcmSTDevOri	:= lIcmSTDev					// Arnazena o valor original pois ï¿½ alterado para legislaï¿½ï¿½o ICMS/PR 2017
local lIcmDevolOri	:= lIcmDevol	  				// Arnazena o valor original pois ï¿½ alterado para legislaï¿½ï¿½o ICMS/PR 2017
...
Local lCpoCusEnt	:= SF4->(FieldPos("F4_CUSENTR")) > 0 			//Tratamento para atender o DECRETO Nï¿½ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto Peï¿½as
...
Local lIpiBenef		:= GetNewPar("MV_IPIBENE",.F.) 				//Nota de saï¿½da de retorno com tipo = Beneficiamento. .T.- Serï¿½ gerado na tag vOutro e destacado nas informaï¿½ï¿½es
																		//complementares do danfe e no campo OUTRAS DESPESAS ACESESSORIAS. .F. - Sï¿½ra gerado na tag vIPI e destacado no campo
																		//VALOR IPI do cabeï¿½alho do danfe (procedimento padrï¿½o)
Local lMvImpFecp	:= GetNewPar("MV_IMPFECP",.F.)	                // Imprime FECP
Local lOrgaoPub		:= GetNewPar("MV_NFORGPU",.F.)				//NF-e de remessa nas operaï¿½ï¿½es de aquisiï¿½ï¿½o de ï¿½rgï¿½o pï¿½blico, com entrega em outro ï¿½rgï¿½o pï¿½blico (RICMS SP)
...
Local lDifParc		:= .F.
Local lNfCompl		:= .F.
Local lFCI			:= GetNewPar("MV_FCIDANF",.F.) // Imprime ou nï¿½o os dados da FCI no Xml/Danfe (De acordo com as configuraï¿½ï¿½es necessï¿½rias)
...
Local aSX560      	:= {}
Local aSX513      	:= {}
//Declaraï¿½ï¿½o de Objetos
Local oWSNfe

//Parametro Logico - Define se sera impresso a 2a. Unidade de Medida para operacoes dentro do Paï¿½s - Opcoes: [.T.]-Nï¿½o Imprime ou [.F.]-Imprime (Default)
...
//Alimentaï¿½ï¿½o da tag retTransp
Local nBCTot 		:= 0
Local nALIQTot		:= 0
Local nVLTRIBTot	:= 0
Local aObsCont		:= {}

//Alimentaï¿½ï¿½o do Grupo de Repasse
Local nBRICMSO 	:= 0
Local nICMRETO	:= 0
Local nBRICMSD 	:= 0
Local nICMRETD	:= 0
...
Private lMvNFLeiZF	:= SuperGetMV("MV_NFLEIZF",,.F.)	// Tratamento para a lei da Portaria Suframa nï¿½ 275/2009 para Pis e Cofins do chamado TPIPVV
...
If cTipo == "1"
	...
	For nCountNF := 1 To Len(aCupRefLoj)
		...
		If MsSeek(xFilial("SF2")+cNota+cSerie+cClieFor+cLoja)
				...
				While !SF3->(Eof()) .And. cClieFor+cLoja+cNota+cSerie == SF3->F3_CLIEFOR+SF3->F3_LOJA+SF3->F3_NFISCAL+SF3->F3_SERIE
					...
					If nCont == 1
						Do While !SD2->(Eof ()) .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
								...
								EndIf
								//------------------------------------------------------------------------------------------------
								// Retirado o uso do parametro MV_SIMPREM para que a mensagem sejï¿½ gerado de acordo com o CSOSN
								//------------------------------------------------------------------------------------------------
								dbSelectArea("SF4")
								dbSetOrder(1)
								MsSeek(xFilial("SF4")+SD2->D2_TES)
			...
			//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
			//ï¿½Tratamento temporario do CTe                                            ï¿½
			//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
			If FunName() == "SPEDCTE" .Or. AModNot(SF2->F2_ESPECIE)=="57"
				...
				//ï¿½Tratamento Nota de Servico  ï¿½
			...
			ElseIf FunName() == "SPEDNFSE"
				...
				While !Eof() .And. xFilial("SF3") == SF3->F3_FILIAL .And.;
					...
					//Tipo de RPS - O sistema de BH ainda nï¿½o estï¿½ recebendo Notas Conjugadas
					...
					//ï¿½Pega os impostos de retencao somente quando houver a retenï¿½ï¿½o, ï¿½
					//ï¿½ou seja, os titulos de retenï¿½ï¿½o que existirem                  ï¿½
					...
					//ï¿½Obtem os dados do Serviï¿½o ï¿½
					//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
					aSX560 	 := FwGetSX5("60",SF3->F3_CODISS)
					If len(aSX560) > 0
						//Verifico se a Descriï¿½ï¿½o ï¿½ composta do pedido de Venda ou SX5
						If cDescServ$"1"
							SC6->(dbSetOrder(4))
							SC5->(dbSetOrder(1))
					...
					//ï¿½Verifica se recolhe ISS Retido ï¿½
					...
					//ï¿½Verifica se municipio de prestaï¿½ï¿½o foi informado no pedido ï¿½
			...
			Else
				...
				//ï¿½Para o caso de Nota sobre Cupom Fiscal, busca os dados da Nota  ï¿½
				...
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
				//ï¿½Posiciona cliente ou fornecedor                                         ï¿½
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½	
				If !SF2->F2_TIPO $ "DB" 
				    dbSelectArea("SA1")
					dbSetOrder(1)
					...
					*/
					If !lUsaCliEnt
						...
						If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))			
							If Len(aEntrega) > 0											
								//Se a UF da entrega for diferente da UF do emitente (operaï¿½ï¿½o interestadual) e o CNPJ do destinatario for diferente do emitente, 
								//tenho que buscar os dados do cliente de entrega para nao ocorrer 
								//rejeicao 523 - CFOP nï¿½o ï¿½ de Operaï¿½ï¿½o Estadual e UF emitente igual ï¿½ UF destinatï¿½rio
								...
								EndIf
								//Se a UF de entrega for igual a UF do emitente (Operaï¿½ï¿½o interna) - busco os dados do cliente para montar como destinatario.
								//Se o CNPJ do emitente for igual ao do destinatï¿½rio tambï¿½m levo os dados do cliente, mesmo que UFs forem diferente.
								//Se o cliente nï¿½o for consumidor final e possuir IE, pode ocorrer a rejeiï¿½ï¿½o 773 - Operaï¿½ï¿½o Interna e UF de destino difere da UF do emitente
					...
					Else
						/* Se MV_NFEDEST estiver habilitado (.T.):
							A tag de destinatï¿½rio - 'dest' serï¿½ gerada com os dados do CLIENTE (F2_CLIENTE)
							Caso possua Cliente de Entrega (F2_CLIENT) a tag de entrega serï¿½ gerada exatamente com os dados do Cliente de Entrega 
							Caso possua Cliente de Retirada (F2_CLIRET) a tag de retirada serï¿½ gerada exatamente com os dados do Cliente de Retirada (***FUTURA IMPLEMENTAï¿½ï¿½O*** campo F2_CLIRET inexistente)
						*/
						If !Empty(cCliNota+cCliLoja)
							SA1->(MsSeek(xFilial("SA1")+cCliNota+cCliLoja))   //Busca os dados do cliente da Nota sobre Cupom para montar os dados do destinatário do XML
						Else
					...
					//Customizado - Para lojas que estÃ£o em outros estados e possuem somente 1 CNPJ e entrega diferente
					//20/08/2019
					If !Empty(alltrim(SF2->F2_XMENS))
						cMensFis += "Endereco de Entrega :"+SF2->F2_XMENS
					EndIf
					//Fim da customizaÃ§Ã£o
					...
					aadd(aDest,SA1->A1_EMAIL)
					aAdd(aDest,SA1->A1_CONTRIB) // Posiï¿½ï¿½o 17
					aadd(aDest,Iif(SA1->(FieldPos("A1_IENCONT")) > 0 ,SA1->A1_IENCONT,""))
					aadd(aDest,SA1->A1_INSCRM)
					aadd(aDest,SA1->A1_TIPO)
					aadd(aDest,SA1->A1_PFISICA)//21-Identificaï¿½ï¿½o estrangeiro
											
				Else
				    dbSelectArea("SA2")
					dbSetOrder(1)
					// Tratamento para quando existir um cliente de entrega, utilizï¿½-lo ao invï¿½s do fornecedor (apenas por garantia)
					If !Empty(AllTrim(SF2->F2_CLIENT)) .And. !Empty(AllTrim(SF2->F2_LOJENT))
				...
				If !Empty(SF2->F2_TRANSP)
					...
					If (SA4->A4_TPTRANS <> "3")
					//Conforme RICMS/MG, Anexo V Art. 2ï¿½, na emissï¿½o do documento fiscal em relaï¿½ï¿½o ao quadro transportador, 
					//se o mesmo for o prï¿½prio remetente ou destinatï¿½rio, deve-se informar a palavra Remetente ou Destinatï¿½rio, 
					//dispensado o preenchimento dos campos: condiï¿½ï¿½o de pagamento, CNPJ ou CPF do transportador, endereï¿½o, municï¿½pio, 
					//unidade da Federaï¿½ï¿½o e a inscriï¿½ï¿½o estadual do transportador (CHAMADO-TVMWZL).
				...
				If MsSeek(cChave)
					...
					// Verifica se o CFOP ï¿½ de venda por consignaï¿½ï¿½o mercantil (CFOP 5111 ou 6111)
					...
					elseif ( AllTrim(SF3->F3_CFO) == "5949" .or. AllTrim(SF3->F3_CFO) == "5910" ) .and. SM0->M0_ESTENT == 'SP' /*termos do inciso II do art. 456 do RICMS/ SP  chamado THPXGS*/ 
						//lBrinde := .T. //Retirado tratamento de brinde pois foi constatado pela consultoria tributï¿½ria que nao e' possivel amarrar por CFOP.
					EndIf
					...
					If lSimpNac
						...
						If SF2->F2_TIPO == "D"
							...
							cMensFis += "Base de cï¿½lculo do ICMS: R$ " + Str(SF2->F2_BASEICM, 14, 2) + ". "
				...
				dbSelectArea("SF2")
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
				//ï¿½Volumes / Especie Nota de Saida                                         ï¿½
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
				cScan := "1"
				While ( !Empty(cScan) )
					...
					While !Eof() .And. xFilial("SE1") == (cAliasSE1)->E1_FILIAL .And.;
						...
						SF2->F2_DOC == (cAliasSE1)->E1_NUM     
							If (cAliasSE1)->E1_TIPO = MVNOTAFIS .OR. (cAliasSE1)->E1_TIPO = 'DP' .OR. ((Alltrim((cAliasSE1)->E1_ORIGEM) $ 'LOJA701|FATA701|LOJA010') .AND. (cAliasSE1)->E1_TIPO $ cWhere)
								...
								//devido a titulos com desconto da TAXA do Cartï¿½o de Crï¿½ito que nï¿½o devem
								...
								If cValLiqB == "2" // 1 Baixa - 2 Emissao									
										//Neste caso nï¿½o subtraio o valor do INSS do titulo. //DSERTSS1-9986
				...
				EndIf
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
				//ï¿½Analisa os impostos de retencao                                         ï¿½
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
				//Tratamento para notas sobre cupom(Incluir demais estados conforme conforme legislacao).
				//A Nota Fiscal  deve ser toda preenchida, sendo a sua escrituraï¿½ï¿½o feita com valores zerados, jï¿½ que o dï¿½bito serï¿½ feito pelo cupom
				//Assim, no livro Registro de Saï¿½das deve ser registrado para esta nota apenas a coluna "Observaï¿½ï¿½es", onde serï¿½o indicados o seu nï¿½mero e a sua sï¿½rie.
				//Fundamento: artigo 135, ï¿½ 2ï¿½, do RICMS/2000.		  	
				//Fundamento: Decreto nï¿½ 29.907/2009 , art. 36 , ï¿½ï¿½ 9ï¿½ e 10ï¿½; RICMS-CE/1997 , art. 731-E1	
				//Fundamento: Portaria SEFP nï¿½ 799, de 30.12.1997 - DO DF de 31.12.1997
				...
				While !Eof() .And. xFilial("SD2") == (cAliasSD2)->D2_FILIAL .And.;
					...
					nCount++
					//Se for nota sobre cupom, pega somente os itens do cupom que estï¿½o na nota sobre cupom.				
					...
					If (cAliasSD2)->D2_TIPO == "D" .And. SM0->M0_ESTENT == "PR" .And. (cAliasSD2)->D2_ICMSRET > 0
						/* Tratamento para com base na legislaï¿½ï¿½o do Estado do Paranï¿½ Decreto n 6.080/2012 - DOE PR Suplemento  
						e para atender a ICMS/PR 2017 (Decreto 7.871/2017) Art. 9, Seï¿½ï¿½o I, Anexo IX 
						que nï¿½o prevï¿½ o destaque do ICMS no campo especï¿½fico (tanto o da operaï¿½ï¿½o prï¿½pria do substituto quanto do 
						retido por substituiï¿½ï¿½o tributï¿½ria) ISSUE DSERTSS1-5542. */						
						lIcmSTDev	:= .F.
						lIcmDevol	:= .F.						
						lIcmsPR	:= .T.
					else
					...
					If lContinua
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Verifica a natureza da operacao                                         ï¿½
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
						If lNfCup
							aAreaSD2  	:= SD2->(GetArea())
							//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
							//ï¿½Pesquisa itens de nota                                                  ï¿½
							//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½	
							...
							RestArea(aAreaSD2)
							// ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½|
							//ï¿½       Informacoes do cupom fiscal referenciado              |
					    	//|                                                             ï¿½
						...
						Else
							...
							//ï¿½Quando nao for cupom fiscal,							ï¿½
							//ï¿½	o CFOP deve ser atualizado com o CFOP de cada ITEM, |
						...
						//Tratamento para atender o DECRETO Nï¿½ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto Peï¿½as
	          ...
	          // Posiciono na TES do cupom fiscal para pegar a natureza de operaï¿½ï¿½o da nf sobre cupom
						/*Necessario para imprimir a natureza de operaï¿½ï¿½o do cupom fiscal emitido em ECF*/
			    		...
			    		Endif
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Verifica as notas vinculadas                                            ï¿½
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
						If !Empty((cAliasSD2)->D2_NFORI)
							If (cAliasSD2)->D2_TIPO $ "DBN"
								...
								If ( MsSeek(xFilial("SD1")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+PADL(alltrim((cAliasSD2)->D2_ITEMORI),TamSx3("D2_ITEMORI")[1],"0")) ) .OR. ;
									...
									EndIf
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
									//ï¿½Obtem os dados de nota fiscal de produtor rural referenciada                                  ï¿½
									//ï¿½Temos duas situacoes:                                                                         ï¿½
									//ï¿½A NF de saï¿½da ï¿½ uma devolucao, onde a NF original pode ser ou nao uma devoluï¿½ï¿½o.              ï¿½
									//ï¿½1) Quando a NF original for uma devolucao, devemos utilizar o remetente do documento fiscal,  ï¿½
									//ï¿½    podendo ser o sigamat.emp no caso de formulario proprio ou o proprio SA1 no caso de nf de ï¿½
									//ï¿½    entrada com formulario proprio igual a NAO.                                               ï¿½
									//ï¿½2) Quando a NF original NAO for uma devolucao, neste caso tambem pode variar conforme o       ï¿½
									//ï¿½    formulario proprio igual a SIM ou NAO. No caso do NAO, os dados a serem obtidos retornara ï¿½
									//ï¿½    da tabela SA2.                                                                            ï¿½
									...
									Endif  
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
									//ï¿½Outros documentos referenciadosï¿½
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
									if AllTrim(SF1->F1_ESPECIE)<>"NFP"
										//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
										//ï¿½Documento de Estorno - Tipo Devolucao e F4_AJUSTE="S"    ï¿½
										//ï¿½identifica que se trata de nf de estorno.                ï¿½
									...
									dbSelectArea("SFT")
							   		...
							   		If MsSeek(xFilial("SFT")+"S"+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_NFORI)
										...
										ElseIf Alltrim(SFT->FT_CFOP) $ cMVREFNFE
											//Tratamento para que leve na TAG <refNFe> as notas referenciadas que contï¿½m o CFOP no parï¿½metro MV_REFNFE
											dbSelectArea("SF3")
									   		dbSetOrder(4) 
										...
										Else
											dbSelectArea("SF3")
									   		...
									   		If MsSeek(xFilial("SF3")+SFT->FT_CLIEFOR+SFT->FT_LOJA+SFT->FT_NFISCAL+SFT->FT_SERIE)										   		
												//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
												//ï¿½Obtem os dados de nota fiscal de produtor rural referenciada                                  ï¿½
												//ï¿½A NF de saï¿½da Para este tipoï¿½
												//ï¿½ de nota, o emitente eh sempre o sigamat.emp                                                  ï¿½
												//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
		...
		//						If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
								If MsSeek(xFilial("SD2")+(cAliasSD2)->D2_NFORI+(cAliasSD2)->D2_SERIORI)//+(cAliasSD2)->D2_CLIENTE+(cAliasSD2)->D2_LOJA+(cAliasSD2)->D2_COD+(cAliasSD2)->D2_ITEMORI)
									...
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
									//ï¿½Obtem os dados de nota fiscal de produtor rural referenciada                                  ï¿½
									//ï¿½A NF de saï¿½da NAO EH uma devolucao, portanto eh uma nota de saida complementar. Para este tipoï¿½
									//ï¿½ de nota, o emitente eh sempre o sigamat.emp                                                  ï¿½
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
                        ...
                        //Atualiza a Unid. Medida da DIPI(cUmDipi) e o Fator de Conv. da DIPI(nConvDip) com dados da SBZ caso os parï¿½metro recebidos estejam vazios
						...
						//impresso os valores da lei da transparï¿½ncia.					
						if lNfCup
							cTpCliente := "F"
						EndIf
						...
						If !AllTrim(SC5->C5_MENNOTA) $ cMensCli
							...
							//-- Tratamento para a integraï¿½ï¿½o entre WMS Logix X ERP Protheus 
						...
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Obtem os dados do veiculo informado no pedido de venda                  ï¿½
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½							
						If Empty(aVeiculo)
							DbSelectArea("DA3")
							DbSetOrder(1)
							...
							EndIf
						EndIf
						//O campo F4_FORINFC ï¿½ um substituto do F4_FORMULA, e atravï¿½s do parï¿½metro MV_NFEMSF4 se determina se o conteï¿½do da formula devera compor a mensagem do cliente (="C") ou do fisco (="F"). 				
						...
						//Indicador de presenï¿½a do comprador no estabelecimento comercial no momento da operaï¿½ï¿½o - VERSï¿½O 3.10
						If SC5->(FieldPos("C5_INDPRES")) > 0
							 If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							 	cIndPres := "1" //1=Operaï¿½ï¿½o presencial
							 Else
						...
						EndIf
						// Tags xPed e nItemPed (controle de B2B) para nota de saï¿½da
						...
						//ï¿½ Conforme Decreto RICM, N 43.080/2002 valido somente em MG deduzir o 	ï¿½ 
						//ï¿½	imposto dispensado na operaï¿½ï¿½o				  			                ï¿½
						...
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//Alteraï¿½ï¿½o realizada no campo F4_ICMSDIF, foi incluido a opï¿½ï¿½o: 6 ï¿½ Diferido(Deduz NF e Duplicata)
						...
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½ Incluido o tratamento pelo fato do SIGALOJA e o VENDA DIRETA nao gravar ï¿½ 
						//ï¿½	o campo D2_DESCON, quando e' dado desconto no total da venda.           ï¿½
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
						If lNfCup .Or. (cAliasSD2)->D2_ORIGLAN $ "VD|LO"
							...
							If SF2->F2_DESCONT > 0
								If lFirstItem	// Somente faz o looping nos itens na primeira vez
									...
									/*Retirado tratamento pois nï¿½o funciona para DBF
									nX := 1
						...
						//ï¿½ Tratamento para verificar se o produto e controlado por terceiros (IDENTB6)ï¿½
						//ï¿½  e a partir do tipo do pedido (Cliente ou Fornecedor) verifica  se existe  ï¿½
						//ï¿½  amarracao entre Produto X Cliente(SA7) ou Produto X Fornecedor(SA5)       ï¿½  
						//ï¿½Caso haja a amarraca, o codigo e descricao do produto, assumem o conteudo   ï¿½
						//ï¿½	da SA7 ou SA5															   ï¿½ 
						...
						//						cCodProd  := (cAliasSD2)->D2_COD	            			- Excluido Linha - BACIO - JEREMIAS - 26/01/17
                        //						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)     - Excluido Linha - BACIO - JEREMIAS - 26/01/17

				        // incrementando na descricao do produto a 2. U.M. na nota fiscal customizacao BACIO - Jeremias - 26/01/17 - INICIO
					
						cCodProd  := (cAliasSD2)->D2_COD	            
                 	    //	s_Segum	  := SD2->D2_SEGUM --DEVIDO ERRO GERADO NO DIA 04122017 MUDEI PARA SC6 
                 	    s_Segum := SC6->C6_SEGUM
						s_DSegum  := POSICIONE('SAH',1,xFilial("SAH")+s_Segum,"AH_UMRES")    
           			    //	s_QtSegum := SD2->D2_QTSEGUM --DEVIDO ERRO GERADO NO DIA 04122017 MUDEI PARA SC6
						s_QtSegum := SC6->C6_QTDENT2                                                                                    
						
						IF s_Segum <> " "
							cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI)+" ****** "+AllTrim(Transform(s_QtSegum,"@ze 999,999.99"))+" "+alltrim(s_DSegum)+"(S)"
						ELSE
						cDescProd := IIF(Empty(SC6->C6_DESCRI),SB1->B1_DESC,SC6->C6_DESCRI) 
						ENDIF
					
						// incrementando na descricao do produto a 2. U.M. na nota fiscal customizacao BACIO - Jeremias - 26/01/17 - FIM
			            ...
			            // Faz o destaque do IPI nos dados complementares caso seja uma venda por consignaï¿½ï¿½o mercantil e possuir IPI
						If (lConsig .Or. Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM) .And. (cAliasSD2)->D2_VALIPI > 0
							nIPIConsig += (cAliasSD2)->D2_VALIPI
						EndIf
							
						// Faz o destaque do ICMS ST nos dados complementares caso seja uma venda por consignaï¿½ï¿½o mercantil e possuir ICMS ST
						If Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM .And. (cAliasSD2)->D2_ICMSRET > 0 .And. lConsig    
							nSTConsig += (cAliasSD2)->D2_ICMSRET 
						EndIf  	
			            
			            //Tratamento para que o valor de ICMS ST venha a compor o valor da tag vOutros quando for uma nota de Devoluï¿½ï¿½o, impedindo que seja gerada a rejeiï¿½ï¿½o 610.
			            nIcmsST := 0
						...
						//-----------------------------------------------------------------------------------------
						//			FCI - Ficha de Conteï¿½do de Importaï¿½ï¿½o
						//-----------------------------------------------------------------------------------------
						//**Operaï¿½ï¿½o INTERNA:
						//1) Emitente da NF (vendedor) Nï¿½O realizou processo de industrializaï¿½ï¿½o com a mercadoria:
						// - Informar o valor da importaï¿½ï¿½o      (Revenda)
						//2) Emitente da NF (vendedor) REALIZOU processo de industrializaï¿½ï¿½o com a mercadoria:
						// - Informar o valor da importaï¿½ï¿½o      (Industrializaï¿½ï¿½o)
						//
						//**Operaï¿½ï¿½o INTERESTADUAL:
						//1) Emitente da NF (vendedor) Nï¿½O realizou processo de industrializaï¿½ï¿½o com a mercadoria:
						// - Informar o valor da importaï¿½ï¿½o      (Revenda)
						//2) Emitente da NF (vendedor) REALIZOU processo de industrializaï¿½ï¿½o com a mercadoria:
						// - Informar o valor da parcela importada do exterior, o nï¿½mero da FCI e o Conteï¿½do de
						//   Importaï¿½ï¿½o expresso percentualmente (Industrializaï¿½ï¿½o)
						//----------------------------------------------------------------------------------------- 
						...
						If (cOrigem $"1-2-3-4-5-6-8" .And. cCSTrib $ "00-10-20-30-40-41-50-51-60-70-90")
							If (cAliasSD2)->(FieldPos("D2_FCICOD")) > 0 .And. !Empty((cAliasSD2)->D2_FCICOD)
								...
								If lFCI
									cMsgFci	:= "Resolucao do Senado Federal nï¿½ 13/12"
						...
						Else 
							aadd(aFCI,{})
						EndIf
							    // Retirada a validaï¿½ï¿½o devido a criaï¿½ï¿½o da tag nFCI (NT 2013/006)
							    //--------------------------------------------------------------------------------
								//Campo SD2->D2_FCICOD sï¿½ ï¿½ preenchido nos casos de Industrializaï¿½ï¿½o Interestadual
								//Executar UPDSIGAFIS para criaï¿½ï¿½o do campo na D2 e tabela CFD.
								//Obs.: O campo D2_FCICOD ï¿½ alimentado com o conteï¿½do do campo CFD_FCICOD apï¿½s
								//faturar os Documentos de Saï¿½da (MATA461).
								//--------------------------------------------------------------------------------
								//If AliasIndic("CFD")
									//CFD->(DbSetOrder(3))   //Tabela de Ficha de Conteudo de Importaï¿½ï¿½o
									//If CFD->(DbSeek(xFilial("CFD")+(cAliasSD2)->D2_FCICOD))
										//-----------------------------------------------------------------------------------
										//Obs.: Retirado o valor da parcela importada devido ao Convï¿½nio 38/2013  CH: THHDRV
								...
								//EndIf
								//--------------------------------------------------------------------------------
								//Preencher o campo C6_VLIMPOR com o valor da Importaï¿½ï¿½o para popular o D2_VLIMPOR
								//Obs.: Somente preencher nos casos em que nï¿½o utilize RASTRO, caso utilize serï¿½
						...
						//Adequaï¿½ï¿½o NT2013/003 - Verifica se o valor serï¿½ composto da tabela SBZ ou SB1
						nAliqNcm := 0
			            ...
			            //Verifica fonte carga tributï¿½ria
						...
						EndIf
			          	...
			          	//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½  Indicador de Produï¿½ï¿½o em escala relevante, conforme Clï¿½usula 23 do Convenio ICMS 52/2017
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ 						
						If AliasIndic("D3E")
							dbSelectArea("D3E")
							dbSetOrder(1)
						...
						nValOutr  := 0
						//Outras despesas + PISST + COFINSST  (Inclusï¿½o do valor de PIS ST e COFINS ST na tag vOutros - NT 2011/004).E devoluï¿½ï¿½o com IPI. (Nota de compl.Ipi de uma devoluï¿½ï¿½o de compra(MV_IPIDEV=F) leva o IPI em voutros)	
						IF((cAliasSD2)->D2_TIPO == "D" .And. !lIpiDev) .Or. lConsig .Or. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM ) .or. ((cAliasSD2)->D2_TIPO == "B" .and. lIpiBenef) .or. ((cAliasSD2)->D2_TIPO=="P" .And. lComplDev .And. !lIpiDev)
							//Observaï¿½ï¿½o para nota de devoluï¿½ï¿½o independente do parametro  MV_IPIDEV / MV_IPIBENE nï¿½o levarï¿½ para  a tag <vOutro> e sim para a tag <vIPIDevol>  Ipi Devolvido ou tag <vIPI> tag prï¿½pria de Ipi.
						...
						aadd(aProd,	{Len(aProd)+1,;
							...
							"",; //aprod[42] apenas na entrada ï¿½ utilizado para montar a tag indPres=1 para nota de devoluï¿½ï¿½o de venda
							nValIFecp,; //aprod[43]  Valor do FECP.
							cCodlan,; //aprod[44]  Cï¿½digo de Benefï¿½cio Fiscal na UF aplicado ao item .
							IIf(SB5->(ColumnPos("B5_2CODBAR")) > 0,IIf(Val(SB5->B5_2CODBAR)==0,"",StrZero(Val(SB5->B5_2CODBAR),Len(Alltrim(SB5->B5_2CODBAR)),0)),""),;//aprod[45]  Cï¿½digo de barra da segunda unidade de medida.
						...
						cNCM := SB1->B1_POSIPI
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Tratamento para TAG Exportaï¿½ï¿½o quando existe a integraï¿½ï¿½o com a EEC     ï¿½
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
						If lEECFAT
							/*Alteraï¿½ï¿½es TQXWO2
							Na chamada da funï¿½ï¿½o, foram criados dois novos parï¿½metros: 
							o 3ï¿½ referente ao cï¿½digo do produto e o 4ï¿½ referente ao nï¿½mero da nota fiscal + sï¿½rie (chave).
							GetNfeExp(pProcesso, pPedido, cProduto, cChave)
							No retorno da funï¿½ï¿½o serï¿½o devolvidas as informaï¿½ï¿½es do legado, conforme leiaute anterior ï¿½ versï¿½o 3.10 , 
							e as informaï¿½ï¿½es dos grupos ï¿½I03 - Produtos e Serviï¿½os / Grupo de Exportaï¿½ï¿½oï¿½ e ï¿½ZA - Informaï¿½ï¿½es de Comï¿½rcio Exteriorï¿½, conforme estrutura da NT20013.005_v1.21.
							As posiï¿½ï¿½es 1 e 2 mantï¿½m o retorno das informaï¿½ï¿½es ZA02 e ZA03, mantendo o legado para os cliente que utilizam versï¿½o 2.00
							Na posiï¿½ï¿½o 3 passa a ser enviado o agrupamento do ID I50, tendo como filhos os IDs I51 e I52.
							Na posiï¿½ï¿½o 4 passa a ser enviado o agrupamento do ZA01, tendo como filhos os IDs ZA02, ZA03 e ZA04.
							Na posiï¿½ï¿½o 5 passa a ser enviado informaï¿½ï¿½oes para o grupo "BA02 - Chaves Nfe referenciadas" as chaves de notas fiscais de saï¿½da de lote de exportaï¿½ï¿½o associadas ï¿½ nota de saï¿½da de exportaï¿½ï¿½o.
							O array de retorno serï¿½ multimensional, trazendo na primeira posiï¿½ï¿½o o identificador (ID), 
							na segunda posiï¿½ï¿½o a tag (o campo) e na terceira posiï¿½ï¿½o o conteï¿½do retornado do processo, 
							podendo ser um outro array com a mesma estrutura caso o ID possua abaixo de sua estrutura outros IDs. 						 				
							*/
							/*Alteraï¿½ï¿½es TUSHX4
							Foi incluido o parametro D2_LOTECTL para que a funï¿½ï¿½o localize as notas de entrada (produto com lote e endereï¿½amento) amarradas no pedido de exportï¿½ï¿½o e consiga
							retornar o array de exportind de acordo com a quantidade de cada item da SD2, para nï¿½o ocorrer a rejeiï¿½ï¿½o 
							346 Somatï¿½rio das quantidades informadas na Exportaï¿½ï¿½o Indireta nï¿½o correspondem a quantidade do item.*/
						...
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Tratamento para Rastreamento de Lote - Cabecalho e Itens   
						//Primeiro busca no compl. de rastreabilidade (F0A) e  depois compl.de medicamento (CD7)                ï¿½
						...
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Tratamento para Anfavea - Cabecalho e Itens                             ï¿½
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½				
						If lAnfavea
							//Cabecalho
							aAnfC := {}
					    ...
					    If SFT->(DbSeek(xFilial("SFT")+"S"+(cAliasSD2)->(D2_SERIE+D2_DOC+D2_CLIENTE+D2_LOJA+PadR(D2_ITEM,TamSx3("FT_ITEM")[1])+D2_COD)))
						   ...
						   EndIf
						   //TRATAMENTO DA AQUISIï¿½ï¿½O DE LEITE DO PRODUTOR RURAL CONFORME ARTIGO 207-B, INCISO II RICMS/MG
						...
						While CD2->(!Eof()) .And. xFilial("CD2") == CD2->CD2_FILIAL .And.;
							...
							// Verifica se existe percentual de reducao na SFT referï¿½te ao RICMS 43080/2002 MG.
							...
							Do Case
								...
								Case AllTrim(CD2->CD2_IMP) == "SOL"
									...
									lCalSol := .T.
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
									//ï¿½Tratamento CAT04 de 26/02/2010                       ï¿½
									//ï¿½Verifica de deve ser garavado no xml o valor e base  ï¿½
									//ï¿½de calculo do ICMS ST para notas fiscais de devolucaoï¿½
									//ï¿½Verifica o parametro MV_ICSTDEV                      ï¿½
									//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
									nValST 	:= Iif(cVerAmb == "4.00" .and. FindFunction("xFisRetFCP"), xFisRetFCP('4.0','CD2','CD2_VLTRIB'), CD2->CD2_VLTRIB)
									//para a 4.0 devera exibir a informaï¿½ï¿½o Valor do ICMS ST nï¿½o majorado.
								...
								Case AllTrim(CD2->CD2_IMP) == "IPI"
									...
									EndIf
								/*Chamado TTVZJG - Grupo impostoDevol - informar o percentual e valor do IPI devolvido, em notas de devoluï¿½ï¿½o (finNFe =4)
								Incluida a verificaï¿½ï¿½o do campo F4_PODER3=D para os casos de retorno de beneficiamento*/
						...
						//Tratamento para que o valor de PIS ST e COFINS ST venha a compor o valor total da tag vOutros  (NT 2011/004). E devoluï¿½ï¿½o de compra com IPI nï¿½o tributado
						If ((cAliasSD2)->D2_TIPO == "D" .and. !lIpiDev)  .Or. lConsig .Or. (Alltrim((cAliasSD2)->D2_CF) $ cMVCFOPREM ) .OR. ((cAliasSD2)->D2_TIPO == "B" .and. lIpiBenef) .OR. ((cAliasSD2)->D2_TIPO=="P" .And. lComplDev .And. !lIpiDev)
							//Observaï¿½ï¿½o para nota de devoluï¿½ï¿½o independente do parametro  MV_IPIDEV / MV_IPIBENE nï¿½o levarï¿½ para  a tag <vOutro> e sim para a tag <vIPIDevol>  Ipi Devolvido ou tag <vIPI> tag prï¿½pria de Ipi.
							If cVerAmb >= "4.00" .And. cTPNota == "4"
								aTotal[01] += 0	
		              ...
		              //Tratamento para que o valor de PIS ST,COFINS ST venha a compor o valor total da nota.
						...
						If findfunction( 'ColumnPos' )					
							...
							EndIF 					
						Else 
							MsgInfo("ï¿½ necessï¿½rio a atualizaï¿½ï¿½o do sistema para a expediï¿½ï¿½o mais recente.")
						EndIf
				...
				If Len(aIPIDev)>0
					...
					EndDo
						...
						For nX := 1 To Len(aIPIAux)
							...
							cMensCli += " "
							cMensCli += "(Valor do IPI: R$ "+cValIPI+" - "+"Classificaï¿½ï¿½o fiscal: "+aIPIAux[nX][2]+") "
							cValIPI  := ""
							cNCMAux  := ""
						Next nX
				...
				Endif
				//Tratamento legislacao do Rio Grande do Sul, quando existir intes com ICMS-ST e intens somente com ICMS  prï¿½prio
				If SM0->M0_ESTCOB $ "RS" .And. Len(aICMS) > 0 .And. Len(aICMSSt) > 0 
					cMensCli += MsgCliRsIcm(aICMS,aICMSSt)
				Endif
				//Tratamento legislacao do DF, quando existir intes com ICMS-ST e intens somente com ICMS  prï¿½prio
				If aDest[9] $ "DF" .And. Len(aICMS) > 0 .And. Len(aICMSSt) > 0 
					cMensCli += MsgCliDFIcm(aICMS,aICMSSt)
				Endif
			    
			    //Mensagem para ICMS Particionado - Convï¿½nio ICMS Nï¿½ 51/00,
			    if nValICMParc > 0 .And. nBasICMParc > 0 .And. nValSTParc > 0 .And. nBasSTParc > 0
					...
					cMensFis += "Faturamento Direto ao Consumidor - Convenio ICMS Nï¿½ 51/00, de 15 de setembro de 2000. "
					...
					If !Empty(aEntrega) 
						...
						cMensFis += "Endereï¿½o: "+ConvType(aEntrega[02],125)+", "+ConvType(aEntrega[03],10)+" "+ConvType(aEntrega[04],60)+". " //Rua,Num,Complemento
					...
					Else
						...
						cMensFis += "Endereï¿½o: "+ConvType(aDest[03],125)+" "+ConvType(aDest[04],10)+" "+ConvType(aDest[05],60)+", " //Rua,Num,Complemento
			    ...
			    if lBrinde
			    	...
			    	cMensCli += "Nota Fiscal emitida nos termos do inciso II do art. 456 do RICMS - Nota Fiscal de Aquisiï¿½ï¿½o nï¿½ "+aDocDat[2]+", de "+aDocDat[1]+"."
			    endif 
		...
		Else				
			...
			If SF1->F1_TIPO $ "DB" 
				...
				If !Empty(SA1->A1_MENSAGE) .and. SA1->(Formula(A1_MENSAGE)) <> Nil .and. !EMPTY(SA1->(Formula(A1_MENSAGE)))
					...
					EndIf
				EndIf								
				/* Quando houver uma troca/devoluï¿½ï¿½o (LOJA720) de uma NFC-e no Estado do AM, a tag <infAdFisco> 
				da NF-e de entrada deve conter o motivo de devoluï¿½ï¿½o, nome, endereï¿½o e cpf do cliente
				O campo F1_MOTIVO ï¿½ preenchido na funcao LOJA720 do SIGALOJA */
				...
				onde o Emitente deverï¿½ realizar a inclusï¿½o de recebimento da recusa utilizando
				CFOP 1.201/2.201 - devoluï¿½ï¿½o de venda de produï¿½ï¿½o do estabelecimento; 
				e (ii) 1.410/2.410 - devoluï¿½ï¿½o de venda de produï¿½ï¿½o do estabelecimento em operaï¿½ï¿½o 
				com produto sujeito ao regime de substituiï¿½ï¿½o tributï¿½ria. 
				Bem como incluindo seus dados de Emitente como Destinatï¿½rio.
				...
				F1_DEVMERC (Identifica devoluï¿½ï¿½o de mercadoria que nï¿½o foi entregue ao destinatï¿½rio em
				atendimento ao Artigo 453, I, do RICMS/2000 SP) 
				Tipo Caracter (Combo S=Sim;N=Nï¿½o) Tamanho 1
				
				Ao preencher o campo como S=Sim, os dados do prï¿½prio estabelecimento (Emitente)
				serï¿½o utilizados como destinatï¿½rio no XML e Danfe, ao invï¿½s do cliente padrï¿½o da nota.		
				*/
				...
				If cDevMerc == "S"
					...
					cFoneEmit := IIF(aTelEmit[1] > 0,ConvType(aTelEmit[1],3),"") // Cï¿½digo do Pais
					cFoneEmit += IIF(aTelEmit[2] > 0,ConvType(aTelEmit[2],3),"") // Cï¿½digo da ï¿½rea
					cFoneEmit += IIF(aTelEmit[3] > 0,ConvType(aTelEmit[3],9),"") // Cï¿½digo do Telefone
				...
				Else
					...
					/* Se MV_NFEDEST estiver desabilitado (default .F.) permanece o legado:
					a) Para operaï¿½ï¿½es interestaduais (UF do emitente diferente da UF do Cliente de Entrega) e o CNPJ do Destinatario(Cliente - F1_FORNECE)
						for DIFERENTE do emitente, serï¿½o considerados os dados do CLIENTE DE ENTREGA.  
						- Os dados do Cliente de Entrega serï¿½o gerados na tag de Destinatï¿½rio - 'dest'.
					b) Para operaï¿½ï¿½es internas (UF do emitente igual a UF do Cliente de Entrega) e se o CNPJ do Destinatï¿½rio(Cliente - F1_FORNECE)
						for IGUAL ao do emitente, serï¿½o considerado os dados do CLIENTE, mesmo que UFs sejam diferentes.
						- Os dados do Cliente serï¿½o gerados na tag de Destinatï¿½rio - 'dest'.
					*/
					If !lUsaCliEnt
						lCNPJIgual := AllTrim(SA1->A1_CGC) == Alltrim(SM0->M0_CGC)				
					...
					Else
						/* Se MV_NFEDEST estiver habilitado (.T.):
							A tag de destinatï¿½rio - 'dest' serï¿½ gerada com os dados do CLIENTE (F1_FORNECE)
							Caso possua Cliente de Entrega (F1_FORENT) a tag de entrega serï¿½ gerada exatamente com os dados do Cliente de Entrega 
							Caso possua Cliente de Retirada (F1_FORRET) a tag de retirada serï¿½ gerada exatamente com os dados do Cliente de Retirada
						*/
						MsSeek(xFilial("SA1")+SF1->F1_FORNECE+SF1->F1_LOJA)
					EndIf
					aadd(aDest,AllTrim(SA1->A1_CGC))
			...
			Else
				...
				Ao preencher o campo como S=Sim, os dados do prï¿½prio estabelecimento (Emitente)
				serï¿½o utilizados como destinatï¿½rio no XML e Danfe, ao invï¿½s do fornecedor padrï¿½o da nota*/
				...
				Else
					...
					Else
						aAdd(aDest, "") 
					EndIf	
					aAdd(aDest, "")// Posiï¿½ï¿½o 18 (referente a A1_IENCONT, sendo passado como vazio jï¿½ que nï¿½o existe A2_IENCONT)
					aadd(aDest,SA2->A2_INSCRM)
					aadd(aDest,"")//Posiï¿½ï¿½o 20
					aadd(aDest,SA2->A2_PFISICA)//21-Identificaï¿½ï¿½o estrangeiro
				EndIf
			...
			if ( SF3->(dbSeek(xFilial("SF3")+cSerie+cNota)) ) 
				...
				//ï¿½ 1ï¿½ do artigo 442, do RICMS CE, determina que todos os documentos recebidos pelo Estado
				// que acobertam operaï¿½ï¿½es interestaduais com este Estado deverï¿½o possuir a Inscriï¿½ï¿½o Estadual de Substituto
				If SF3->F3_ESTADO == "CE"					
					If At (SF3->F3_ESTADO, cMVSUBTRIB)>0
						nPosI	:=	At (SF3->F3_ESTADO, cMVSUBTRIB)+2
						nPosF	:=	At ("/", SubStr (cMVSUBTRIB, nPosI))-1
			...
			//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
			//ï¿½Verifica Duplicatas da nota de entrada                                  ï¿½
			//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½		
			...
			dbSelectArea("SF1")
			//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
			//ï¿½Volumes / Especie Nota de Entrada                                       ï¿½
			//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
			cScan := "1"
			...
			If SD1->(FieldPos("D1_DESCZFC")) > 0
				cField += ",D1_DESCZFC"
			EndIf
			If SD1->(FieldPos("D1_GRPCST"))<>0 //Grupo de tributaï¿½ï¿½o de ipi
			   cField  +=",D1_GRPCST"				    
			EndIf
			
			If SD1->( ColumnPos('D1_AFRMIMP') ) > 0 //Campo especï¿½fico para despesa de importaï¿½ï¿½o
			   cField  +=",D1_AFRMIMP"				    
			EndIf			
			...
			While !Eof() .And. xFilial("SD1") == (cAliasSD1)->D1_FILIAL .And.;
	    		...
	    		//Operaï¿½ï¿½o com diferimento parcial de 66,66% do RICMS/PR para importaï¿½ï¿½o
	    		lDifParc := .F.
	    		...
	    		EndIf
	    		//O campo F4_FORINFC ï¿½ o substituto do F4_FORMULA, e atravï¿½s do parï¿½metro MV_NFEMSF4  se determina se o conteudo da formula devera compor a mensagem do cliente(="C") ou do fisco(="F").
				...
				//Verifica se existe Template DCL
     			...
     			// Verifica se o CFOP ï¿½ de devoluï¿½ï¿½o por consignaï¿½ï¿½o mercantil (CFOP 1111/1111/1918/2918)
				...
				/*Tratamento para NF DE AJUSTE chamado THYZ13 -  PORTARIA Nï¿½ 163/2007 Artigo 18-B-2 item 4a da SEFAZ-MT */
				...
				If !Empty((cAliasSD1)->D1_NFORI)
                	...
                	//Ajustes para que ao gerar nota de entrada do tipo complemento de preï¿½o de uma devoluï¿½ï¿½o seja vinculado o cliente correto 
					//da nota de origem.                		
					dbSelectArea("SD1")
					dbSetOrder(1)
					...
					//Alterado a chave de busca completa devido ao procedimento de complemento de notas de devolucao de VENDA onde o codigo do fornecedor nï¿½o seja o mesmo da nota de origem 
					...
					Else
						...
						ELSE 
							...
							if !(SF4->F4_AJUSTE=='S' .and. ((cAliasSD1)->D1_TIPO == "D")) .and. cSpecie <> 'NFCE' .And. !DevCliEntr(cAliasSD1)
					   		...
					   		else  /* Quando for uma devoluï¿½ï¿½o de Ajuste nï¿½o tem necessidade de informar os outros campo para posicionar na SD2. chamado TIANDL*/
					   			cMsSeek	:=(cFilDev+(cAliasSD1)->D1_NFORI+(cAliasSD1)->D1_SERIORI)
					   		endif       
						EndIF                                                                          
				...
				Else
					//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
					//ï¿½Verifica as notas vinculadas na tabela SF8 - Amarracao NF Orig x NF Imp/Fre     ï¿½
		       ...
		       EndIf   		
				...
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
				//Tratamento para verificar se o produto ï¿½ controlado por terceiros (IDENTB6)ï¿½
				//e a partir do tipo do documento (Cliente ou Fornecedor) verifica  se existeï¿½
				//amarracao entre Produto X Cliente(SA7) ou Produto X Fornecedor(SA5)       ï¿½  
				//Caso haja a amarracao, o codigo e descricao do produto, assumem o conteudo  ï¿½
				//da SA7 ou SA5															   ï¿½ 
				//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½  
				...
				If (((cAliasSD1)->D1_TIPO == "D" .And. !lEipiDev) .Or. ((cAliasSD1)->D1_TIPO == "B" .and. lIpiBenef))
					//Observaï¿½ï¿½o para nota de devoluï¿½ï¿½o independente do parametro  MV_EIPIDEV / MV_IPIBENE nï¿½o levarï¿½ para  a tag <vOutro> e sim para a tag <vIPIDevol>  Ipi Devolvido ou tag <vIPI> tag prï¿½pria de Ipi.        		            							
					If cVerAmb >= "4.00" .And. cTPNota == "4"
						nValOutr += 0		
					Else
				...
				If lEasy  .And. !Empty((cAliasSD1)->D1_TIPO_NF)
					...
					If !cTipoNF$"IPC" .And. cTipoNFEnt <> "6"
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Tratamento para TAG Importaï¿½ï¿½o quando existe a integraï¿½ï¿½o com a EIC  (Se a nota for primeira ou unica)|
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
						aadd(aDI,(GetNFEIMP(.F.,cDocEnt,cSerEnt,cFornece,cLojaEnt,cTipoNFEnt,cPedido,cItemPC,cLote,cItem)))
					Else
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
						//ï¿½Tratamento para TAG Importaï¿½ï¿½o quando existe a integraï¿½ï¿½o com a EIC  (Se a nota for complementar)     |
						//ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½
						aadd(aDI,(GetNFEIMP(.F.,cNFOri,cSerOri,cFornece,cLojaEnt,cTipoNFEnt, ,cItemOri, ,cItem )))
					EndIf
					aAdi := aDI
				// Se nï¿½o o parï¿½metro de integraï¿½ï¿½o entre o SIGAEIC e o SIGAFAT estiver desabilitado,
				//   procura as informaï¿½ï¿½es da importaï¿½ï¿½o da tabela CD5 (complemento de importaï¿½ï¿½o).
				ElseIf !lEasy .Or. (lEasy .AND. (cAliasSD1)->D1_TIPO $ "DCPI")
					...
					If MsSeek(xFilial("CD5")+(cAliasSD1)->D1_DOC+(cAliasSD1)->D1_SERIE+(cAliasSD1)->D1_FORNECE+(cAliasSD1)->D1_LOJA+(cAliasSD1)->D1_ITEM)
						...
						aAdi := aDI
					// Caso nenhum registro de complemento de importaï¿½ï¿½o para essa nota exista, coloca os arrays em branco
					Else
						aadd(aAdi,{})
						aadd(aDi,{})											
					EndIf
					...
					Do Case
						Case AllTrim(CD2->CD2_IMP) == "ICM"
							aTail(aICMS) := {CD2->CD2_ORIGEM,;
											  CD2->CD2_CST,;
											  CD2->CD2_MODBC,; 
							                  nMargem,;// Tratamento para obter o percentual da reduï¿½ï¿½o de base do icms nota interna e importacao(integracao com EIC)							                  
							CD2->CD2_BC,;
	           ...
	           //Tratamento para que o valor de PIS ST e COFINS ST venha a compor o valor total da tag vOutros  (NT 2011/004). E devoluï¿½ï¿½o de compra com IPI nï¿½o tributado apenas para saida
				//Tratamento para que ao transmitir uma nota de devoluï¿½ï¿½o leve o valor do IPI conforme configurado o parametro MV_EIPIDEV.
			...
			//INSERE MSG EM INFADFISCO E SOMA NO TOTAL DA NOTA.
			If nValLeite > 0 .And. nPercLeite > 0
				cMensFis += Alltrim(Str(nPercLeite,10,2))+'% Incentivo ï¿½ produï¿½ï¿½o e ï¿½ industrializaï¿½ï¿½o do leite = R$ '+ Alltrim(Str(nValLeite,10,2))
				aTotal[02] += nValLeite
			EndIf
			
			//Operaï¿½ï¿½o com diferimento parcial de 66,66% do RICMS/PR
			If nValIcmDev > 0 .And. nValIcmDif > 0
				...
				cMensFis += "nos termos do Art 459 do DECRETO N.ï¿½ 7.871/2017 - RICMS/PR" //ISSUE DSERTSS1-6543 - Decreto 7.871 que revoga o regulamento do ICMS aprovado pelo decreto n 6080 de 28 de setembro de 2012.
			Endif
	...
	IF cVeramb >= "4.00"
	   //Ajute para alimentar o aDetPag
		...
		ElseIf (lVLojaDir .OR. !Empty(POSICIONE("SL1",2,xFilial("SL1")+SF2->F2_SERIE+SF2->F2_DOC,"L1_NUM"))) .And. cTipo == "1" .And. ( aRetPgLoj := GetPagLoja(aDupl, cChvPag, aTotal[2], cIndPag,cVerAmb) )[1]
			...
			aDetPag := aRetPgLoj[2]	
		Else		
			//caso tenha escolhido a forma de pagamento no cadastro de condiï¿½ï¿½o de pagamento.
			dbSelectArea("SE4")
			dbSetOrder(1)	
...
If !Empty(aNota)
	...
	IF cVeramb >= "4.00"
		//Obrigatï¿½rio o preenchimento do Grupo Informaï¿½ï¿½es de Pagamento para NF-e e NFC-e. Para as notas com finalidade de Ajuste ou Devoluï¿½ï¿½o o
		//campo Forma de Pagamento deve ser preenchido com 90=Sem Pagamento.
		//Retirado o grupo de duplicata para nï¿½o ocorrer a Rejeiï¿½ï¿½o 867: Grupo duplicata informado e forma de pagamento nï¿½o ï¿½ Duplicata Mercantil.
...
//Nota Tï¿½cnica 2013/005 - Data e Hora no formato UTC
If cVeramb >= "3.10"
...
If cVeramb >= "3.10"
	...
	If SubStr(cCFOP,1,1) == "2" .Or. SubStr(cCFOP,1,1) == "6" 
		 cOper:= "2" //Operaï¿½ï¿½o Interestadual
	ElseIf SubStr(cCFOP,1,1) == "3" .Or. SubStr(cCFOP,1,1) == "7" 
		cOper:= "3" //Operaï¿½ï¿½o com Exterior
	Else
		cOper:= "1" //Operaï¿½ï¿½o Interna CFOP 1 e 5
	EndIf

	//Operaï¿½ï¿½o Interna/Interestadual, pois apesar de CFOP 3 ou 7, porem UF de cliente diferente de EX/ Rejeiï¿½ï¿½o 731 e 520 (NT 2013/005 v 1.10)/(NT 2010/007)
	...
	//Identificador de Local de Destino da Operaï¿½ï¿½o
...
If(!Empty(aNfVinc)	.And. Empty(aExp[1])) .or.(!Empty(aNfVinc).And. !Empty(aExp[1]) .and. lEECFAT)
	...
	For nX := 1 To Len(aNfVinc)
		...
		// Verifica se ja foi gerada a tag para a mesma nota anteriormente, para nï¿½o ser gerada novamente
		//   ocasionando em rejeiï¿½ï¿½o pela SEFAZ
...
if SM0->M0_ESTCOB  ==  'RS' .and. anota[5] == 'C' .and. len(aNfVincRur) > 0   // verifica se estado ï¿½ RS e se nota ï¿½ complemento
	aNfVincRur := FiltEst(@aNfVincRur, SM0->M0_ESTCOB ) // remove notas referenciadas que nï¿½o sï¿½o do RS
endif
...
If !Empty(aRefECF)
	If len(aRefECF) > 0 .and. cVerAmb >= "2.00"        
		...
		For nX := 1 To Len(aRefECF)
			// Verifica se ja foi gerada a tag para o mesmo ECF / CF, para nï¿½o ser gerada novamente
			// ocasionando em rejeiï¿½ï¿½o pela SEFAZ
...
/*Quando hï¿½ exportaï¿½ï¿½o indireta(I52), deve-se informar as chaves(I54) na tag refNFe.
EEC nï¿½o consegue preencher campo D2_NFORI pois pode existir mais de um documento de entrada para referenciar em um mesmo item,
por este motivo, as chaves recebidas na tag chNFe do grupo exportInd serï¿½o geradas automaticamente na refNFe.
*/

If !Empty(aExp[1]) .and. lEECFAT .and. cVerAmb >= "3.10"
	If Len(aExp) > 0 .and. (aNota[04] == "1" .OR.  aNota[5] $ "D|N") //Somente se nota de saï¿½da ou devoluï¿½ï¿½o.
		For nX := 1 To Len(aExp)
			If Len(aExp[nX][3][3]) > 0
				For nY := 1 To Len(aExp[nX][3][3][2])
					//Quando nï¿½o hï¿½ exportInd, a posiï¿½ï¿½o 3 ï¿½ retornada vazia
				...
				Next nY
			EndIf 
         /*Quando hï¿½ notas fiscais de formaï¿½ï¿½o de lote de exportaï¿½ï¿½o associadas, deve-se informar as chaves(BA02) na tag refNFe.
			  Estas informaï¿½ï¿½es ficarï¿½o na posiï¿½ï¿½o 6 do array aExp retornado peloa funï¿½ï¿½o GETNFEEXP( ) do SIGAEEC - Exportaï¿½ï¿½o */
	...
	EndIf	
/*SEM INTEGRAï¿½ï¿½O COM EEC - Quando hï¿½ exportaï¿½ï¿½o indireta, deve-se informar a chave(I54 - tag chNFe) na tag refNFe.
Caso nï¿½o seja vinculada a NF original no pedido de venda (C6_NFORI/D2_NFORI), serï¿½ considerada a chave contida
...
/*Verificaï¿½ï¿½o do conteudo da tag IndIeDest para atribuir valor 1 na tag indFinal - rej 696-NT2015/003_v1.71*/
If ConvType(aDest[17]) <> "2" .and. !Empty(aDest[14])
	...
	EndIf
Else
	cIndicador := "9" //9-Nï¿½o Contribuinte
EndIf
//Ajuste para considerar o tipo do cliente cadastrado na (C5_TIPOCLI) quando alterado no cabeï¿½alho das NF de Saï¿½da
//pois todos os cï¿½lculos fiscais sï¿½o feitos com base nessa informaï¿½ï¿½o e nï¿½o no campo A1_TIPO.
...
If cVeramb >= "3.10"
	If cTipocli  == "F"
		cString += '<indFinal>1</indFinal>' //1-Operaï¿½ï¿½o com consumidor final
		cIndFinal:= "1"
	Else
		If cIndicador == "9" .and. cIdDest <> "3" //(tag indIEDest=9)-Nï¿½o Contribuinte e operaï¿½ï¿½o que nï¿½o ï¿½ com exterior (tag idDest <> 3)
			cString += '<indFinal>1</indFinal>'//1-Consumidor final
			cIndFinal:= "1"
		Else
			cString += '<indFinal>0</indFinal>'//0-Nï¿½o
			cIndFinal:= "0"
		EndIf
	EndIf
	If Empty(cIndPres)
		...
		If aNota[5] == "N"
			cIndPres := "9" //Operaï¿½ï¿½o nï¿½o presencial 
		ElseIf aNota[5] == "D" .and. aNota[04] == "0" .and. (!Empty(cVENPRES) .and. cVENPRES == "1")
			/*Manutenï¿½ï¿½o para considerar o conteï¿½do do campo F4_VENPRES=1 na montagem da tag 
				indPres = 1 ï¿½ Operaï¿½ï¿½o Presencial, em notas de devoluï¿½ï¿½o de venda para contribuinte de 
				outro Estado, com CFOP iniciado por 1 e sem frete, a fim de nï¿½o apresentar a 
				rejeiï¿½ï¿½o 521 - Operaï¿½ï¿½o Interna e UF do emitente difere da UF do destinatï¿½rio/remetente 
				contribuinte do ICMS.*/
			cIndPres := "1"

		Else
			cIndPres := "0" //0-Nï¿½o se Aplica	
		EndIf
	EndIf
	cString += '<indPres>'+cIndPres+'</indPres>' // Presenï¿½ï¿½o do comprador no momento da Operaï¿½ï¿½o
EndIf
cString += '</ide>'
...
/*
Quando utilizar Gestao de empresas o M0_NOME guarda o nome do Grupo e nï¿½o da Filial.
FWFilialName - Pega o nome da Filial Atual,sï¿½ usar funcao se estiver habilitado 
...
cFoneDest := IIF(aTelDest[1] > 0,ConvType(aTelDest[1],3),"") // Cï¿½digo do Pais
cFoneDest += IIF(aTelDest[2] > 0,ConvType(aTelDest[2],3),"") // Cï¿½digo da ï¿½rea
cFoneDest += IIF(aTelDest[3] > 0,ConvType(aTelDest[3],9),"") // Cï¿½digo do Telefone
cString += NfeTag('<fone>',cFoneDest)
cString += '</enderEmit>'
cString += '<IE>'+ConvType(VldIE(SM0->M0_INSC))+'</IE>'
If !Empty(aIEST) 
	/*ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ä¿
	  ï¿½ Tratamento para acordo entre os estados preenchidos no parametro MV_STNIEUF, quando em      ï¿½
	  ï¿½ um movimento com ICMS-ST nao e' necessario ter insccricao estadual, assim esse tratamento   ï¿½
	  ï¿½ retorna a inscricao " " para gerar a guia de recolhimento para o estado destino             ï¿½ 
	  ï¿½ Este tratamento foi feito a partir da necessidade das UF de MG p/ PR,onde existe esse 	    ï¿½
	  ï¿½ acordo PROTOCOLO ICMS CONSELHO NACIONAL DE POLï¿½TICA FAZENDï¿½RIA - CONFAZ Nï¿½ 191 DE 11.12.2009ï¿½ 
	...
	/*Inserida a verificaï¿½ï¿½o do idDest = 2 por conta de rejeiï¿½ï¿½o
	347 Informada IE do substituto tributï¿½rio em operaï¿½ï¿½o que nï¿½o ï¿½ interestadual
...
Static Function NfeDest(aDest,cVerAmb,aTransp,aCST,lBrinde,cMunDest)
	...
	cString := '<dest>'
	If cVerAmb >= '3.10'
	//Estrangeiro nï¿½o manda a tag de CPFCNPJ
		If !"EX"$aDest[09]
	...
	If cVerAmb >= "3.10"
		...
		/*	indIEDest - Indicador da IE do destinatï¿½rio
			1=Contribuinte ICMS (informar a IE do destinatï¿½rio);
			2=Contribuinte isento de Inscriï¿½ï¿½o no cadastro de Contribuintes do ICMS;
			9=Nï¿½o Contribuinte, que pode ou nï¿½o possuir Inscriï¿½ï¿½o Estadual no Cadastro de Contribuintes do ICMS;	
		*/
	Else
		// Conforme legislaï¿½ï¿½o, nï¿½o contribuinte em SP, deve levar a IE se preenchida.
	...
	//Tratamento para atender Manual de Orientaï¿½ï¿½o do Contribuinte versï¿½o 5.00 onde ï¿½ Obrigatï¿½rio, nas operaï¿½ï¿½es que se beneficiam de incentivos fiscais existentes nas ï¿½reas sob controle da SUFRAMA.
...
//senï¿½o verifica se a unidade comercial e diferente da tributaria para considerar o mesmo valor da cEan se for igual.
cEantrib	:= IIF(!Empty(aProd[45]),aProd[45], iif( aProd[08] <> aProd[11],"",cEan))

//Validaï¿½ï¿½o para controle das tags que deverï¿½o ser preenchidas apenas nas operaï¿½ï¿½esï¿½nï¿½o destinadas a consumidor final RS
...
// Define se o valor do produto <vProd> serï¿½ agregado ao valor total
//   dos produtos do documento <vProd> dentro de <total>
cString += '<indTot>'+aProd[24]+'</indTot>'

/* Adequaï¿½ï¿½o Nota Tï¿½cnica 2013/003 (Obs. Tratamento apenas para documento de saï¿½da pois refere-se a venda ao consumidor) */
If cTipo == "1" .And. cTpCliente == "F"
	cString += NfeTag('<vTotTrib>' ,ConvType(aProd[30],15,2))
EndIf


/*Nas situaï¿½ï¿½es em que o valor unitï¿½rio comercial (vUnCom) for diferente do valor unitï¿½rio tributï¿½vel (vUnTrib), 
ambas as informaï¿½ï¿½es deverï¿½o estar expressas e identificadas no DANFE - CH:TGCOQA*/
...
//ï¿½Criaï¿½ï¿½o de novo grupo ï¿½Rastreabilidade de produtoï¿½ para permitir a rastreabilidade de qualquer produto sujeito a regulaï¿½ï¿½es sanitï¿½rias.
...
//Ver II - Average - Tag da Declaraï¿½ï¿½o de Importaï¿½ï¿½o aDI
If Len(aDI)>0 .And. ConvType(aDI[04][1]) == "I19"
	...
	EndIf
	cString += '</DI>'
	/*Impressï¿½o dos dados da DI nas informaï¿½ï¿½es complementares do Danfe - CH:TELKDV*/
	If lDInoDanfe
		cMsgDI  := " "
...
/*Grupo de informaï¿½ï¿½es de exportaï¿½ï¿½o para o item - versï¿½o 3.10*/
If cVerAmb >= "3.10" .and. Len(aExp)>0
	If lEECFAT
		/*Quando a terceira posiï¿½ï¿½o do array estiver vazia ou possuir tamanho 0 ï¿½ porque a informaï¿½ï¿½o nï¿½o existe no processo.
		  Quando nï¿½o houver dados de retorno referente ao ato concessï¿½rio e a exportaï¿½ï¿½o indireta, a posiï¿½ï¿½o [3][3] terï¿½ tamanho 0.
		  Quando houver ato concessï¿½rio, a informaï¿½ï¿½o serï¿½ retornada na posiï¿½ï¿½o [3][3][1]. O tamanho dessa dimensï¿½o corresponde ï¿½ quantidade de atos concessï¿½rios encontrados para o item.
		  Quando houver exportaï¿½ï¿½o indireta, a informaï¿½ï¿½o serï¿½ retornada na posiï¿½ï¿½o [3][3][2]. O tamanho dessa dimensï¿½o corresponde ï¿½ quantidade de notas fiscais de remessa com fim especï¿½fico de exportaï¿½ï¿½o encontrada para o item.
		*/
		If ConvType(aExp[03][1]) == "I50" .and. Len(aExp[03][3]) > 0
				...
				For nA:= 1 to Len(aExp[03][3][1])
					...
					//Caso nï¿½o tenha I52, posiï¿½ï¿½o 3 ï¿½ retornada vazia
		...
		EndIf
	// Para nota de devoluï¿½ï¿½o de exportaï¿½ï¿½o gerar a tag  exportInd para nï¿½o ocorrer a rejeiï¿½ï¿½o:340
...
If Len(aComb) > 0  .And. !Empty(aComb[01])
	...
	Endif	
	/*NT 2015/002
	379 - Rejeiï¿½ï¿½o: Grupo de Encerrante na NF-e (modelo 55) para CFOP diferente 
	de venda de combustï¿½vel para consumidor final (CFOP=5.656, 5.667).	
	*/
...
If !Empty(aveicProd) .And. !Empty(aveicProd[02])
	...
	//Alteraï¿½ï¿½o efeutada para permitir que de acorodo com o Manual NFE 6.0, 
	...
	cString += '<espvei>'+SubStr(aveicProd[19],2,1)+'</espvei>'  // Considera apenas a segunda posiï¿½ï¿½o do campo CD9_ESPVEI
...
If (aCST[01] = "60" .or. cCsosn$ "500") .and. !Empty(cUltAqui)  .and. Len(aIcms)>19
	//novos campos na tabela SFT sï¿½o: FT_BSTANT (Base) FT_PSTANT (Percentual) FT_VSTANT (Valor) Atenciosamente.
	nBaseIcm :=  aICMS[20]      //SFT->FT_BSTANT
	nValICM  :=  aICMS[21]      //SFT->FT_VSTANT
	nAlqICM  :=  aICMS[22]      //SFT->FT_PSTANT
	//novos campos do FECP na tabela SFT sï¿½o: FT_BFCANTS (Base) FT_PFCANTS (Percentual) FT_VFCANTS (Valor) Atenciosamente.
...
If  !lIssQn    
	If cMVCODREG == "1" .and. SF4->(FieldPos("F4_CSOSN"))>0 .And. !Empty(SF4->F4_CSOSN)
	    ...
	    Elseif cCsosn$"201,202,203,900"
			...
			IF cVeramb >= "4.00" .AND. Len(aICMSST)>0 
			   IF cCsosn$ "201"
				...
				//pCredSN Alï¿½quota aplicï¿½vel de cï¿½lculo do crï¿½dito
             // vCredICMSSN Valor crï¿½dito do ICMS	
		...
		If cCsosn$"500"   
			...
			//Tratamento implementado para atender o DECRETO Nï¿½ 54.308, DE 6 DE NOVEMBRO DE 2018. (publicado no DOE n.ï¿½ 212, de 7 de novembro de 2018)					
	...
	Else
		...
		If Len(aIcms)>0
			...
			// No caso de diferimento (CST 51) o cliente que deverï¿½ escolher a opï¿½ï¿½o 90
			//   caso esteja utilizando a versï¿½o 2.00 da NF-e, enquanto nï¿½o houver adequaï¿½ï¿½o.
			// o sistema nï¿½o pode forï¿½ar o CST 90
			...
			Else
				If cVerAmb >= "3.10" .and. aCST[1] $ '51' .and. !Empty(aICMS[12]) .and. !lArt186
					...
					/*Na versï¿½o 3.10, para CST=51, O Valor do ICMS(vICMS) deve ser a diferenï¿½a do Valor do ICMS da Operaï¿½ï¿½o (vICMSOp) e o Valor do ICMS diferido (vICMSDif),
					para nï¿½o apresentar a rejeiï¿½ï¿½o 353-Valor do ICMS no CST=51 nï¿½o corresponde a diferenï¿½a do ICMS operaï¿½ï¿½o e ICMS diferido*/
				ElseIf cVerAmb >= "3.10" .and. aCST[1] $ '51' .and. Empty(aICMS[12]) .and. Empty(aICMS[07])
					...
					cString += '<valor>0</valor>'
					nVIcmDif += 0
				//Regra para quando nï¿½o tiver icms diferido com CST=51 (F4_icmsdif = 2 e F4_picmdif = 0) 
				ElseIf cVerAmb >= "3.10" .and. aCST[1] $ '51' .and. Empty(aICMS[12]) .and. !Empty(aICMS[07])
					cString += NfeTag('<vICMSOp>' ,ConvType(iIf(lIcmDevol,aICMS[07],0),15,2))
					cString += '<pDif>0</pDif>'
					cString += '<vICMSDif>0</vICMSDif>'
	...
	If Len(aIcmsST)>0	
		...
		//Criaï¿½ï¿½o de campos relativos ao FCP (Fundo de Combate ï¿½ Pobreza) para operaï¿½ï¿½es internas ou interestaduais com ST.
	...
	ELse
		...
		If (aCST[01] = "60" .or. cCsosn$ "500")		
			//Se o parï¿½metro  MV_ULTAQUI que trata a ï¿½ltima aquisiï¿½ï¿½o nï¿½o estiver preenchido usa o modelo antigo de rastro.
			If Empty(cUltAqui) 
				SPEDRastro2(aProd[20],aProd[19],aProd[02],@nBaseIcm,@nValICM,,,lCalcMed,@nAlqICM,,,,,,,,,,,@nBfcpant,@nAfcpant,@nVfcpant)
			...
			If nBaseIcm > 0 .and. nValICM > 0	.and. nAlqICM > 0
			    ...
			    If cArt274 == "1"  .And. IIF(!lEndFis,ConvType(SM0->M0_ESTCOB),ConvType(SM0->M0_ESTENT)) == "SP"
					// cMensFis += "Imposto Recolhido por Substituiï¿½ï¿½o - Artigo 274 do RICMS (Lei 6.374/89, art.67,Paragrafo 1o., e Ajuste SINIEF-4/93',clï¿½usa terceira, na redaï¿½ï¿½o do Ajuste SINIEF-1/94) 'Cod.Produto:  " +ConvType(aProd[02])+" ' Valor da Base de ST: R$ " +str(nBaseIcm,15,2)+" Valor de ICMS ST: R$ "+str(nValICM,15,2)+" "
	...
	//A sefaz nao permite referenciar nota de difal antes de 2016 da a Rejeiï¿½ao 699  
	If len(aItemVinc) > 0 
	...
	Elseif Len(aICMUFDest) == 0 .and. (cIdDest == "2" .and. cIndFinal == "1" .and. cIndIEDest == "9") .and. (Len(aISSQN)== 0) .and. (cAmbiente == "2" .or. (cAmbiente == "1" .and. FsDateConv(aNota[03],"YYYY") >= "2016" .and. lDateRefNf)) .and. (iif(Len(aComb) > 0 .And. !Empty(aComb[01]),aComb[01] $ NfeCodANP(),.T.))
		...
		If (valType(aPPDifal)== "A" .and. Len(aPPDifal)>0 .and. Year(aNota[03]) >= aPPDifal[1][1])
			...
			IF !Empty(aItemVinc) .and. (nPos := aScan(aPPDifal,{|x| x[1]== Year(aItemVinc[01])})) > 0 // Verifica o ano da nota vinculada para pegar a aliquota do parï¿½metro
...
if !empty(aProd[15]) .And. !empty(cMotDesICMS) .or. ( !empty(cMotDesICMS).and. lIcmDevol .and. !Empty(nDesonICM) ) /*Conforme chamado TILXYR foi incluido o .OR. para incluir devoluï¿½ï¿½o na mensagem*/
...
// Grupo opcional 'impostoDevol' para informar o valor e percentual do IPI devolvido para notas de Devoluï¿½ï¿½o
...
//Alteraï¿½ï¿½o para que o valor de PIS ST e COFINS ST venha a compor o valor da nota este valor se encontra na tag vOutros  (NT 2011/004). E devoluï¿½ï¿½o de compra com IPI nï¿½o tributado
...
Essa funï¿½ï¿½o insere o MD-5 do PAFLISTA.TXT
...
If Len( aNfVinc ) > 0		//Nota de espï¿½cie NFE ou NCE ou CTE vinculada
	For nZ := 1  to Len( aNfVinc )
		...
		EndIf
	Next nZ
ElseIf Len( aRefECF ) > 0	 	//Nota de espï¿½cie ECF vinculada
	...
	For nX := 1  to Len( aRefECF )
		...
		EndIf
	Next Nx
ElseIf Len( aNfVincRur ) > 0 	//Nota de espï¿½cie NFP vinculada 
...
//Tratamento para atender o DECRETO Nï¿½ 35.679, de 13 de Outubro de 2010 - Pernambuco para o Ramo de Auto Peï¿½as - TRGTM2
If lCustoEntr
	cString += "ICMS apurado nos termos do Decreto nº 35.679, de 13 de Outubro de 2010."
EndIf

//Tratamento para adcionar o valor do ICMS desonerado para informaï¿½ï¿½o complementar da Danfe.
If nVicmsDeson >0
...
// Tratamento TAG Exportaï¿½ï¿½o integraï¿½ï¿½o com EEC Average 
If Len(aExp)>0 .And. !Empty(aExp[01])
	If lEECFAT
	/*Se versï¿½o 2.00 considera o retorno das posiï¿½ï¿½es 1 e 2
		Se versï¿½o 3.10, considera array da posiï¿½ï¿½o 4 do primeiro item
	*/
		If cVerAmb == "2.00"
			cString += '<exporta>'
			...
			cString += '</exporta>'	
		EndIf
		If ( cTipo == "1") //Somente se nota de saï¿½da ou devoluï¿½ï¿½o.
...
tratamento do texto do C5_MENNOTA quando a origem do PV ï¿½ igual a 'LOGIX'
   ...
   SC5->( dbSetOrder(1) ) 
	...
	If SC5->( MsSeek( cChave := xFilial("SC5") + cPedVinc ) )	
	   ...
	   If SC6->( MsSeek( cChave )  )
	   	//-- Percorre itens de pedido de venda relacionado o nï¿½mero do pedido com a NF , Sï¿½rie e Data
	   ...
	   EndIf   
	EndIf 
	//-- Atualiza mensagem do pedido, @N ( Numero da NF ) ; @S ( Sï¿½rie da NF) ; @D ( Data emissao )
...
arrey com as informaï¿½ï¿½es da nota de NFP
...
@param		aICMS		Array com informaï¿½ï¿½es referente ao ICMS proprio
@param		aICMSST	Array com informaï¿½ï¿½es referente ao ICMS-ST
...
If lIcmsSemSt .And. lIcmsST
	...
	cMsg += "Operaï¿½ï¿½es nï¿½o sujeitas a Regime de ST, "
	cMsg += "Base de Cï¿½lculo do ICMS prï¿½prio: R$ " + Alltrim( Str(nBaseIcm-nBaseST, 14, 2) )+ ", "
	cMsg += "Valor do ICMS prï¿½prio: R$ " + Alltrim( Str(nValIcm-nValST, 14, 2) )+ ". "
	cMsg += "Operaï¿½ï¿½es sujeitas a Regime de ST, " 			
	cMsg += "Base de cï¿½lculo do ICMS prï¿½prio : R$ " + Alltrim( Str(nBaseST, 14, 2) )+ ", "
	cMsg += "Valor do ICMS prï¿½prio: R$ " + Alltrim( Str(nValST, 14, 2) )+ ". "
...
Funcao criada para retornar para a funï¿½ï¿½o XmlNfeSef os valores da Nota Original quando houver controle de SubLote

@param		cNumLote	Nï¿½mero do SubLote.
@param		cLoteClt	Nï¿½mero do lote.
...
@param	nPos 	Posiï¿½ï¿½o ref. Aliq. Tributante: 30 - Aliquota Total
...
Verifica se nota de devoluï¿½ï¿½o utiliza cliente de entrega da nota de origem.
...
Verifica se nota de complemento de preco e se a nota origem estï¿½ na base
...
Funï¿½ï¿½o que monta o grupo autXML da NFe
...
Funï¿½ï¿½o que verifica se o cï¿½digo ANP permitido para gerar o grupo ICMSUFDes para
nï¿½o ocorrer a  Rejeiï¿½ï¿½o. 695 :Informado indevidamente o grupo de ICMS para a UF de destino.

@param		Nil  
@return    cString	String contendo os codigos ANP permitidos para gerar o grupo ICMSUFDes.
                       
                        Operaï¿½ï¿½o com combustï¿½vel (tag:comb) derivado de petrï¿½leo:
                        cï¿½digo ANP diferente de: 
...
Funï¿½ï¿½o  para gerar a mensagem do FECP  por estado -DF - MG - PR - RJ - RS 
         utilizado em informacoes complementares da nota
      
@param		nVfecp	      Valor numï¿½rico do FECOP referente ao valor total
@param		nVfecp	      Estado da Filial Corrente.
@param		cFinalid  //'1' Retorna a mensagem no campo "Informaï¿½ï¿½es Complementares"
                       //'2' Retorna a mensagem no campo "Informaï¿½ï¿½o Adicional do Produto"
                     
@return    cString	String contendo a mensagem  "Informaï¿½ï¿½es Complementares" 
                        ou "Informaï¿½ï¿½o Adicional do Produto Conforme o cFinalid "
...
Do Case
  // MG: "Arquivo Decreto nï¿½ 46.927, de 29 de dezembro de 2015.docx", pï¿½gina 3:
  //Art. 6ï¿½ Nas operaï¿½ï¿½es sujeitas ao adicional de alï¿½quota, o contribuinte indicarï¿½ no campo ï¿½Informaï¿½ï¿½es 
  //Complementaresï¿½ da nota fiscal a expressï¿½o ï¿½Adicional de alï¿½quota ï¿½ Fundo de Erradicaï¿½ï¿½o da Misï¿½ria
  //ï¿½ acompanhada do respectivo valor.
	...
	IF cVerAmb == "4.00" 
		...
		If cDestDanf =='1' // Informação do fisco <infAdFisco> para fecp
  		...
  		elseIf  cDestDanf =='2' // Informaï¿½ï¿½o do fisco <indAdProd> para fecp
  			If Len(aICMS) > 0 
  			...
  			If Len(aICMSSt) > 0 
  				If  (aICMSST[13] > 0 .or. aICMSST[14] > 0 .or.  aICMSST[15] > 0)
  					If cEstado <> "PR"  // Para PR. nï¿½o enviamos o valor 
...
// Tratamento para operacoes dentro do Paï¿½s
If lNoImp2UM
...
//O valor ï¿½ limitado a 4 casas deciamais 
...
Do Case
	...
	Case ((aNota[5]) $ "I-D-C-B" .And. SF4->F4_AJUSTE == "S")
   		cTPNota:= "3"
	/* Referente ao chamado TIDMJV que contempla, nota de transferï¿½ncia de crï¿½dito / dï¿½bito */	   		
	...
	Case (cVeramb >= "3.10" .and. (!Empty(aNfVinc) .Or. !Empty(aRefECF) .Or. !Empty(aNfVincRur))) .and. ( (aNota[5] $ "D|B" .And. SF4->F4_AJUSTE <> "S") .or. (aNota[5] $ "N" .And. SF4->F4_PODER3 == "D") )
   		...
   		If nPos > 0 
   			cTPNota:= "1" 
   		Else
   			cTPNota:= "4" //Devoluï¿½ï¿½o de Mercadoria
   		EndIf
   		
   	 /*Ajuste para emitir notas do tipo devoluï¿½ï¿½o Tag< finnfe> =4  sem necessidade de referenciar a nota original 
     para os  CFOP  1.201, 1.202, 1.410, 1.411, 5,921 e 6,921 . Evitando a rejeiï¿½ï¿½o 321- Rejeiï¿½ï¿½o: NF-e de devoluï¿½ï¿½o de mercadoria nï¿½o possui
...
Grupo ICMS60 (id:N08) informado indevidamente nas operaï¿½ï¿½es
com os produtos combustï¿½veis sujeitos a repasse interestadual
(tag:cProdANP).

@param		Nil  
@return    cString	String contendo os codigos de produto ANP nï¿½o permitidos para gerar o grupo ICMS60 quando cst 60.
...
Retornar informaï¿½ï¿½es de pagamentos feitas via Venda Direta/Sigaloja

@param		aDupl     Duplicatas para o caso de nï¿½o ser enviada nenhuma forma de pagamento, para entao fazer-se a validaï¿½ï¿½o para os tipos  //outros e //14=Duplicata Mercantil 
@param		cChvPag   Condiï¿½ï¿½o de pagamento
...
Local cA1Cgc	 := ""
Local lCondCN	 := .F.	//Condiï¿½ï¿½o Negociada
Local nTroco	 := 0
...
Else
	...
	If SL1->(dbseek(xFilial('SL1') + SF2->F2_SERIE + SF2->F2_DOC))
		...
		If SL1->L1_CREDITO > 0
			isloja	:= .T.			
					    //{cForma          , Valor          , Troco, Tp Integra, CGC Adm Cartao, Cod Bandeira, Autorizaï¿½ï¿½o TEF}
			aadd(aDetPag, {GetFormPgt("CR"), SL1->L1_CREDITO, 0.00 , "2"       , ""            , ""          , "", cIndPag}) 
			
			//Caso em conjunto com a NCC tenha sido utilizada condiï¿½ï¿½o de pagamento cadastrada na SE4 que nao seja negociada, ja trato aqui
	...
	If !Empty(cCondPag)
		Do Case
			...
			Case cCondPag == "CR"//CREDITO LOJA
				cForma := "05"
			Case cCondPag == "VA"//VALE ALIMENTAï¿½ï¿½O
				cForma := "10"
			Case cCondPag == "VR"//VALE REFEIï¿½ï¿½O
				cForma := "11"
...
@param		cCondPag  Condiï¿½ï¿½o de pagamento entre "R$"//DINHEIRO, "CH"//CHEQUE, "CC" //CARTAO DE CREDITO, "CD"//CARTAO DE DEBITO AUTOMATICO, "CR"//CREDITO LOJA
				        						  "VA"//VALE ALIMENTAï¿½ï¿½O, "VR"//VALE REFEIï¿½ï¿½O, "VP"//VALE PRESENTE, "VC"//VALE COMBUSTIVEL, "DM"//Duplicata Mercantil
												  "BOL" //BOLETO BANCARIO, "SPG" //SEM PAGAMENTO
@param		aDupl     Duplicatas para o caso de nï¿½o ser enviada nenhuma forma de pagamento, para entao fazer-se a validaï¿½ï¿½o para os tipos  //outros e //14=Duplicata Mercantil 
...
Remove a citaï¿½ï¿½o ï¿½ nota fiscal complementar que seja diferente do estado do emissor 

@param		aRef      Array que contï¿½m as notas de referï¿½ncia

@param		cEst	   Estado do Emissor (obtido do SM0) 

@return     aRet    Possui notas de referï¿½ncia que podem ser citadas no xml (que sï¿½o do mesmo estado que o emissor).
...
Retorna a Unid. Medida da DIPI e o Fator de Conv. da DIPI da SBZ caso os parï¿½metro recebidos estejam vazios.

@param  cProduto - Produto que serï¿½ localizado na SBZ
